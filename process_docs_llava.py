# process_docs_llava.py
"""
Processing pipeline script for LLaVA-enhanced document processing with
comprehensive safety checks and verification features
"""
import os
import sys
import argparse
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("process_docs_llava.log"), logging.StreamHandler()]
)
logger = logging.getLogger("process_docs_llava")
...
def verify_setup():
    ...
    print("=" * 60)
    print("SYSTEM VERIFICATION FOR LLAVA PROCESSING")
    print("=" * 60)
...
pydantic>=2.0.0""")
    ...
    print("\nSetup verification complete. System is ready for LLaVA processing.")
    return True

def run_llava_pipeline(skip_processing=False, skip_embeddings=False, force=False):
    """Run the complete LLaVA-enhanced processing pipeline."""
    start_time = time.time()
    
    print("=" * 60)
    print("SMC DOCUMENTATION Q&A SYSTEM - LLAVA PROCESSING PIPELINE")
    print("=" * 60)
    ...
    if not skip_processing:
        ...
        if force and os.path.exists(os.path.join(PROCESSED_DIR, "chunked_docs_llava.pkl")):
            os.remove(os.path.join(PROCESSED_DIR, "chunked_docs_llava.pkl"))
            print("Forced removal of existing processed documents.")
        
        # Run LLaVA document processor
        from document_processor_llava import process_directory
        ...
        if not os.path.exists(os.path.join(PROCESSED_DIR, "chunked_docs_llava.pkl")):
    ...
    if not skip_embeddings:
        ...
        from embeddings_llava import process_embeddings_and_db
        try:
    ...
    print("\nLLAVA PROCESSING PIPELINE COMPLETE")
    print("-" * 60)
    print(f"Total processing time: {elapsed_time:.2f} seconds")
    print(f"LLM: Ollama with {OLLAMA_MODEL} model")
    print(f"Vision: Ollama with {LLAVA_MODEL} model")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LLaVA-Enhanced Document Processing Pipeline")
